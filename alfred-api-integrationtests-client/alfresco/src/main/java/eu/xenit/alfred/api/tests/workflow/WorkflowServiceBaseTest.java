package eu.xenit.alfred.api.tests.workflow;
...
public abstract class WorkflowServiceBaseTest extends JavaApiBaseTest {
    ...
    protected IWorkflowService alfredApiWorkflowService;
    ...
    public void initialiseBeansWorkflowServiceBaseTest() {
        alfredApiWorkflowService = getBean("eu.xenit.alfred.api.workflow.IWorkflowService", IWorkflowService.class);
    }
    ...
    protected boolean alfredApiTaskHasNoOwner(Task task) {
        ...
        boolean alfredApiTaskHasNoOwner = null == ownerValue || ownerValue.isEmpty();
        logger.debug("alfredApiTaskHasNoOwner: " + alfredApiTaskHasNoOwner);
        if (!alfredApiTaskHasNoOwner) {
            logger.debug("alfredApiTaskHasNoOwner ownerValue is " + ownerValue);
        } else if (null != ownerValue && ownerValue.isEmpty()) {
            logger.debug("alfredApiTaskHasNoOwner ownerValue is empty");
        }
        return alfredApiTaskHasNoOwner;
    }

    protected boolean probeClaimAndRelease(String taskID, String currentUserName) {
        boolean alfredApiProbeClaimTask = alfredApiProbeClaimTask(taskID, currentUserName);
        final String finalTaskID = taskID;
        ...
        boolean alfredApiProbeReleaseTask = alfredApiProbeReleaseTask(task, currentUserName);
        ...
        logger.debug("probeClaimAndRelease: alfredApiProbeClaimTask: " + alfredApiProbeClaimTask);
        logger.debug("probeClaimAndRelease: alfredApiProbeReleaseTask: " + alfredApiProbeReleaseTask);
        ...
        return alfredApiProbeClaimTask
                && alfredApiProbeReleaseTask
    ...
    protected boolean alfrescoProbeClaimTask(String taskID, String currentUserName) {
        ...
        boolean notAlfredApiIsClaimable = !alfredApiIsClaimable(taskID);
        boolean alfredApiIsReleasable = alfredApiIsReleasable(taskID);
        ...
        logger.debug("alfrescoProbeClaimTask: notAlfredApiIsClaimable: " + notAlfredApiIsClaimable);
        logger.debug("alfrescoProbeClaimTask: alfredApiIsReleasable: " + alfredApiIsReleasable);

        return notAlfrescoIsClaimable
                && alfrescoIsClaimable
                && notAlfredApiIsClaimable
                && alfredApiIsReleasable;
    }
    ...
    protected boolean alfrescoIsClaimable(Task alfredApiWfTask, String username) {
        final String finalTaskID = alfredApiWfTask.getId();
    ...
    protected boolean alfredApiProbeClaimTask(String taskID, String currentUserName) {
        final String finalTaskID = taskID;
        ...
        boolean notAlfredApiIsClaimable = !alfredApiIsClaimable(updatedTask);
        boolean notAlfrescoIsClaimable = !alfrescoIsClaimable(updatedTask, currentUserName);
        boolean alfredApiIsReleasable = alfredApiIsReleasable(updatedTask);
        boolean alfrescoIsReleasable = alfrescoIsReleasable(updatedTask.getId(), currentUserName);

        logger.debug("alfredApiProbeClaimTask: notAlfredApiIsClaimable: " + notAlfredApiIsClaimable);
        logger.debug("alfredApiProbeClaimTask: notAlfrescoIsClaimable: " + notAlfrescoIsClaimable);
        logger.debug("alfredApiProbeClaimTask: alfredApiIsReleasable: " + alfredApiIsReleasable);
        logger.debug("alfredApiProbeClaimTask: alfrescoIsReleasable: " + alfrescoIsReleasable);

        return notAlfredApiIsClaimable
                && notAlfrescoIsClaimable
                && alfredApiIsReleasable
                && alfrescoIsReleasable;
    }

    protected boolean alfredApiIsClaimable(String taskId) {
        final String finalTaskID = taskId;
        Task task = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Task>() {
                    @Override
                    public Task execute() {
                        return alfredApiWorkflowService.getTaskInfo(finalTaskID);
                    }
                }, false, true);
        return alfredApiIsClaimable(task);
    }

    protected boolean alfredApiIsClaimable(Task task) {
        ...
        boolean alfredApiTaskHasNoOwner = alfredApiTaskHasNoOwner(task);

        logger.debug("alfredApiIsClaimable: isClaimable: " + isClaimable);
        logger.debug("alfredApiIsClaimable: alfredApiTaskHasNoOwner: " + alfredApiTaskHasNoOwner);

        return alfredApiTaskHasNoOwner && isClaimable;
    }

    protected boolean alfredApiProbeReleaseTask(String taskId, String currentUserName) {
        final String finalTaskId = taskId;
        ...
        return alfredApiProbeReleaseTask(wfTask, currentUserName);
    }

    protected boolean alfredApiProbeReleaseTask(WorkflowTask wfTask, String currentUserName) {
        final String wfTaskId = wfTask.getId();
        Task releasedWFTask = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Task>() {
                    @Override
                    public Task execute() {
                        return alfredApiWorkflowService.releaseWorkflowTask(wfTaskId);
                    }
                }, false, true);

        boolean notAlfredApiIsReleasable = !alfredApiIsReleasable(releasedWFTask);
        boolean alfredApiIsClaimable = alfredApiIsClaimable(releasedWFTask);
        ...
        logger.debug("alfredApiProbeReleaseTask: notAlfredApiIsReleasable: " + notAlfredApiIsReleasable);
        logger.debug("alfredApiProbeReleaseTask: alfredApiIsClaimable: " + alfredApiIsClaimable);
        logger.debug("alfredApiProbeReleaseTask: notAlfrescoIsReleasable: " + notAlfrescoIsReleasable);
        logger.debug("alfredApiProbeReleaseTask: alfrescoIsClaimable: " + alfrescoIsClaimable);

        return notAlfredApiIsReleasable
                && alfredApiIsClaimable
                && notAlfrescoIsReleasable
                && alfrescoIsClaimable;
    }

    protected boolean alfredApiIsReleasable(String taskId) {
        final String wfTaskId = taskId;
        Task task = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Task>() {
                    @Override
                    public Task execute() {
                        return alfredApiWorkflowService.getTaskInfo(wfTaskId);
                    }
                }, false, true);
        return alfredApiIsReleasable(task);
    }

    protected boolean alfredApiIsReleasable(Task task) {
        if (!task.getProperties().containsKey(ContentModel.PROP_OWNER.toString())) {
            Assert.fail(ContentModel.PROP_OWNER + " is not in the properties of the task.");
        }
        boolean isReleasable = (Boolean) task.getProperties().get(IWorkflowService.ALFRESCO_RELEASABLE);

        logger.debug("alfredApiIsReleasable: isReleasable: " + isReleasable);
    ...
    protected boolean probeCancelWorkflow(WorkflowPath workflow) {
        ...
        Object result = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Object>() {
                    @Override
                    public Object execute() {
                        alfredApiWorkflowService.cancelWorkflow(workflowId);
                        return null;
                    }
                }, false, true);
        ...
        boolean alfredApiIsWorkflowCanceled_DoesNotExist = deletedWorkflow == null;
        logger.debug(
                "probeCancelWorkflow: alfredApiIsWorkflowCanceled_DoesNotExist: " + alfredApiIsWorkflowCanceled_DoesNotExist);

        return alfredApiIsWorkflowCanceled_DoesNotExist;
    }
        ...
        Workflow resultWf = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Workflow>() {
                    ...
                    public Workflow execute() {
                        return alfredApiWorkflowService.getWorkflowInfo(workflowID);
                    }
                }, false, true);
    ...
    protected boolean alfredApiProbeUpdateProperties(String taskId, WorkflowOrTaskChanges changes) {
        logger.debug("alfredApiProbeUpdateProperties: taskId: " + taskId);
        logger.debug("alfredApiProbeUpdateProperties: WorkflowOrTaskChanges: ");
        PrintDebugMapOfString2String("alfredApiProbeUpdateProperties input", changes.getPropertiesToSet());
        ...
        Map<String, Serializable> oldProperties = this.transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Map<String, Serializable>>() {
                    ...
                    public Map<String, Serializable> execute() {
                        return alfredApiWorkflowService.getTaskInfo(finalTaskId).getProperties();
                    }
                }, false, true);
        PrintDebugMapOfString2Serializable("alfredApiProbeUpdateProperties oldProperties", oldProperties);
        ...
        Task updatedTask = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Task>() {
                    @Override
                    public Task execute() {
                        return alfredApiWorkflowService.updateTask(finalTaskId, finalChanges);
                    }
                }, false, true);

        Map<String, Serializable> updatedProperties = updatedTask.getProperties();
        PrintDebugMapOfString2Serializable("alfredApiProbeUpdateProperties updatedProperties", updatedProperties);
        ...
        logger.debug("alfredApiProbeUpdateProperties: resultAllPropertiesApplied: " + resultAllPropertiesApplied);
    ...
    protected boolean alfredApiProbeUpdateProperties(String taskId, WorkflowOrTaskChanges changes,
        ...
        logger.debug("alfredApiProbeUpdateProperties: taskId: " + taskId);
        logger.debug("alfredApiProbeUpdateProperties: WorkflowOrTaskChanges: ");
        PrintDebugMapOfString2String("alfredApiProbeUpdateProperties input changes", changes.getPropertiesToSet());
        PrintDebugMapOfString2String("alfredApiProbeUpdateProperties input unmodifiableProperties", unmodifiableProperties);
    ...
    protected void probeAlfredApiTransitionTask(String transition, String taskID) {
        logger.debug("probeAlfredApiTransitionTask: " + taskID + " : " + transition);
        ...
        this.transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<Map<String, Serializable>>() {
                    @Override
                    public Map<String, Serializable> execute() {
                        alfredApiWorkflowService.endTask(finalTaskID, finalTransitionID);
                        return null;
                    }
                }, false, true);
    }
    ...
    protected void initTransitionsAndTasks(String methodName, String sourceTaskID,
        ...
        Task task = this.transactionHelper
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Task>() {
                    @Override
                    public Task execute() {
                        return alfredApiWorkflowService.getTaskInfo(finalSourceTaskID);
                    }
                }, false, true);
        ...
        return this.transactionHelper.doInTransaction(
                new RetryingTransactionHelper.RetryingTransactionCallback<TaskOrWorkflowSearchResult>() {
                    ...
                    public TaskOrWorkflowSearchResult execute() {
                        return alfredApiWorkflowService.searchTasks(finalSearchQuery);
                    }
                }, false, true);
    }
