...
////                .doInTransaction(() -> service.setMetadata(c.alfredApi(noderef), changes), false, true);
//
...
//                return service.setMetadata(c.alfredApi(noderef), changes);
//            }
//        }, false, true);
//
...
//    }
    ...
    public void TestSetMetadata() {
        ...
        try {
            ...
            eu.xenit.alfred.api.data.NodeRef testNodeRef = c.alfredApi(testNode.getNodeRef());
            eu.xenit.alfred.api.data.NodeRef testNodeRef2 = c.alfredApi(testNode2.getNodeRef());

            eu.xenit.alfred.api.data.QName[] aspectsToRemove = new eu.xenit.alfred.api.data.QName[]{
                    c.alfredApi(ContentModel.ASPECT_VERSIONABLE), c.alfredApi(ContentModel.ASPECT_TEMPORARY)};
            Map<eu.xenit.alfred.api.data.QName, String[]> propertiesToSet = new HashMap<>();
            propertiesToSet.put(c.alfredApi(ContentModel.PROP_NAME), new String[]{"newName"});
            propertiesToSet.put(c.alfredApi(ContentModel.PROP_ADDRESSEES), new String[]{"addressee1", "addressee2"});

            MetadataChanges changes = new MetadataChanges(c.alfredApi(ContentModel.TYPE_CONTENT),
    ...
    public void TestSetAssociations() {
        ...
        try {
            ...
            eu.xenit.alfred.api.data.NodeRef testNodeRef = c.alfredApi(testNode.getNodeRef());
            eu.xenit.alfred.api.data.NodeRef testNodeRef2 = c.alfredApi(testNode2.getNodeRef());
            eu.xenit.alfred.api.data.NodeRef testNodeRef3 = c.alfredApi(testNode3.getNodeRef());

            service.createAssociation(testNodeRef, testNodeRef2, c.alfredApi(ContentModel.ASSOC_ORIGINAL));
            ...
            service.createAssociation(testNodeRef, testNodeRef2, c.alfredApi(ContentModel.ASSOC_ATTACHMENTS));

            service.removeAssociation(testNodeRef, testNodeRef2, c.alfredApi(ContentModel.ASSOC_ORIGINAL));
            ...
            service.createAssociation(testNodeRef, testNodeRef3, c.alfredApi(ContentModel.ASSOC_ORIGINAL));
...
//        java.util.List<NodeMetadata> metadatas = service.getMetadata(c.alfredApi(roots.stream().collect(Collectors.toList())));
...
//        assertTrue(metadatas.stream().allMatch(m -> roots.stream().anyMatch(p -> m.id.equals(p.toString()))));
        ...
        NodeMetadata metadatas = service.getMetadata(c.alfredApi(roots.iterator().next()));
        logger.debug(metadatas.toString().replaceAll(",", ",\n"));

        assertEquals(c.alfredApi(roots.iterator().next()), metadatas.getId());
...
//
        categoryService.classifyNode(c.alfredApi(ref), c.alfredApi(catRef));
    ...
    public void testGetChildAssociations() {
        ...
        try {
            List<ChildParentAssociation> childAssocs = this.service
                    .getChildAssociations(c.alfredApi(testFolder.getNodeRef()));
            assertEquals(childAssocs.size(), 1);
            for (NodeAssociation childAssoc : childAssocs) {
                assertEquals(childAssoc.getSource().toString(), c.alfredApi(testFolder.getNodeRef()).toString());
                assertEquals(childAssoc.getTarget().toString(), c.alfredApi(testNode.getNodeRef()).toString());
    ...
    public void testGetParentAssociations() {
        ...
        try {
            ...
            for (NodeAssociation parentAssoc : parentAssocs) {
                assertEquals(parentAssoc.getSource().toString(), c.alfredApi(testNode.getNodeRef()).toString());
                assertEquals(parentAssoc.getTarget().toString(), c.alfredApi(testFolder.getNodeRef()).toString());
    ...
    public void testGetPeerAssociations() {
        ...
        try {
            ...
            List<NodeAssociation> peerAssociations = this.service.getTargetAssociations(c.alfredApi(testNode.getNodeRef()));
            ...
            peerAssociations = this.service.getSourceAssociations(c.alfredApi(copyNodeRef));
            assertPeerAssociation(testNode.getNodeRef(), copyNodeRef, peerAssociations);
    ...
    public void testCopyNode() {
        ...
        FileInfo testNode = this.createTestNode(mainTestFolder.getNodeRef(), "testnode");
        try {
            logger.debug("sourceNode: " + c.alfredApi(testNode.getNodeRef()));
            logger.debug("targetNode: " + c.alfredApi(testFolder.getNodeRef()));

            eu.xenit.alfred.api.data.NodeRef copiedNodeRef = this.service.copyNode(c.alfredApi(testNode.getNodeRef()),
                    c.alfredApi(testFolder.getNodeRef()),
                    true);
    ...
    public void testMoveNode() {
        ...
        try {
            this.service.moveNode(c.alfredApi(testNode.getNodeRef()), c.alfredApi(testFolder.getNodeRef()));
            assertEquals(this.alfrescoNodeService.getPrimaryParent(testNode.getNodeRef()).getParentRef().toString(),
                    testFolder.getNodeRef().toString());
        } finally {
            this.removeTestNode(mainTestFolder.getNodeRef());
    ...
    public void testCreateNode() {
        ...
        try {
            eu.xenit.alfred.api.data.NodeRef createdNodeRef = this.service
                    .createNode(c.alfredApi(testFolder.getNodeRef()), "testnode", c.alfredApi(ContentModel.TYPE_CONTENT));
    ...
    public void testCreateNodeCustomTypeWithRequiredProperties() {
        ...
        propertiesToSet.put(new eu.xenit.alfred.api.data.QName("{http://test.alfredapi.xenit.eu/model/content}documentStatus"),
                new String[]{"Draft"});

        // custom type containing the mandatory property.
        eu.xenit.alfred.api.data.QName type = new eu.xenit.alfred.api.data.QName(
                "{http://test.alfredapi.xenit.eu/model/content}withMandatoryPropDocument");

        try {
            eu.xenit.alfred.api.data.NodeRef createdNodeRef = this.service
                    .createNode(c.alfredApi(testFolder.getNodeRef()), propertiesToSet, null, null, type, null);
            assertNotNull(createdNodeRef);
            assertEquals(
                    this.alfrescoNodeService.getPrimaryParent(c.alfresco(createdNodeRef)).getParentRef().toString(),
                    testFolder.getNodeRef().toString());
            assertEquals(c.alfredApi(this.alfrescoNodeService.getType(c.alfresco(createdNodeRef))), type);
        } finally {
            this.removeTestNode(mainTestFolder.getNodeRef());
        }
    }
            ...
            serviceRegistry.getRetryingTransactionHelper()
                    .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Void>() {
                        ...
                        public Void execute() throws Throwable {
                            ...
                            propertiesToSet
                                    .put(c.alfredApi(ContentModel.PROP_NAME), new String[]{"newNameWithoutMandatory"});
                            ...
                            self.service.createNode(c.alfredApi(testFolder.getNodeRef()), propertiesToSet, null, null, type,
                                    null);
                            return null;
                        }
                    }, false, true);
    ...
    public void testCreateReadContent() throws Exception {
        ...
        try {
            ...
            Map<eu.xenit.alfred.api.data.QName, String[]> propertiesToSet = new HashMap<>();
            propertiesToSet.put(c.alfredApi(ContentModel.PROP_NAME), new String[]{"nodeWithContent"});
            eu.xenit.alfred.api.data.NodeRef createdNodeRef = this.service
                    .createNode(c.alfredApi(testFolder.getNodeRef()), propertiesToSet, null, null,
                            c.alfredApi(ContentModel.TYPE_CONTENT),
                            content);
    ...
    public void testCreateNodeWithMetadata() throws UnsupportedEncodingException {
        ...
        try {
            ...
            eu.xenit.alfred.api.data.QName documentStatusQname =
                    new eu.xenit.alfred.api.data.QName("{http://test.alfredapi.xenit.eu/model/content}documentStatus");
            ...
            aspectsToAdd[0] = c.alfredApi(ContentModel.ASPECT_TEMPORARY);
            ...
            eu.xenit.alfred.api.data.NodeRef createdNodeRef = self.service.createNode(
                    c.alfredApi(testFolder.getNodeRef()), propertiesToSet, aspectsToAdd, null, type, null);

            assertNotNull(createdNodeRef);
            assertEquals(
                    alfrescoNodeService.getPrimaryParent(c.alfresco(createdNodeRef)).getParentRef().toString(),
                    testFolder.getNodeRef().toString());
            assertEquals(c.alfredApi(alfrescoNodeService.getType(c.alfresco(createdNodeRef))), type);
    ...
    public void testCheckoutCheckin() {
        ...
        try {
            ...
            eu.xenit.alfred.api.data.NodeRef workingCopy = this.service
                    .checkout(c.alfredApi(testNode.getNodeRef()), c.alfredApi(mainTestFolder.getNodeRef()));
    ...
    public void TestFileUpload() throws IOException {
        ...
        try {
            ...
            eu.xenit.alfred.api.data.NodeRef createdNodeRef = service
                    .createNode(c.alfredApi(mainTestFolder.getNodeRef()), testFile.getName(),
                            c.alfredApi(ContentModel.TYPE_CONTENT));
        ...
        try {
            eu.xenit.alfred.api.data.ContentData apContentData =
                    new eu.xenit.alfred.api.data.ContentData(
                            ...
                            "UTF8",
                            Locale.CHINA);
            service.setContent(c.alfredApi(testNode), apContentData);
    ...
    public void TestGetRootNode() {
        assertEquals(
                service.getRootNode(alfredApiStoreRef).toString(),
