...
public class GroupTest extends RestV2BaseTest {
    ...
    public void setup() {
        ...
        transactionService.getRetryingTransactionHelper()
                .doInTransaction(new RetryingTransactionHelper.RetryingTransactionCallback<Object>() {
                    ...
                    public Object execute() throws Throwable {
                        for (int i = 0; i < 3; i++) {
                            c.apix(self.alfrescoPersonService.createPerson(propList.get(i)));
                        }
                        return null;
                    }
                }, false, true);
    ...
    public void testNoUsersInGroup() throws IOException {
        String url = createApixUrl("/groups/%s/people?immediate=true", groupIdentifier);
    ...
    public void testAddUserToGroup() throws IOException {
        String putUrl = createApixUrl("/groups/%s/people", groupIdentifier);
        String getUrl = createApixUrl("/groups/%s/people?immediate=true", groupIdentifier);
