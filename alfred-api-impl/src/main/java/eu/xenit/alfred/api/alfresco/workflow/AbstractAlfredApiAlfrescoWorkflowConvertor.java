...
public abstract class AbstractAlfredApiAlfrescoWorkflowConvertor extends AbstractAlfredApiWorkflowConvertor {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AbstractAlfredApiAlfrescoWorkflowConvertor.class);
    ...
    private static final Map<String, String> AlfredApiToAlfrescoNames = new HashMap<String, String>() {{
    ...
    protected Map<QName, Serializable> filterEditableProperties(WorkflowOrTaskChanges changes) {
        ...
        for (String property : changes.getPropertiesToSet().keySet()) {
            ...
            String normalizedPropertyName = AlfredApiToAlfrescoNames.containsKey(property) ?
                    AlfredApiToAlfrescoNames.get(property) : property;
            ...
            editableProperties
                    .put(this.c.alfresco(new eu.xenit.alfred.api.data.QName(normalizedPropertyName)), normalizedValue);
        }
    ...
    private Serializable normalizePropertyValueToAlfresco(String propId, String value) {
        if (IsIntProperty(propId)) {
            ...
            return (IsNodeRefProperty(propId) ? this.c.alfresco(new eu.xenit.alfred.api.data.NodeRef(value)) : value);
        }
    }

    protected Map<String, Serializable> toAlfredApiProperties(Map<QName, Serializable> props) {
        ...
        for (QName property : props.keySet()) {
            if (property == null) {
                ...
                ret.put(property.toString(), toAlfredApiPropertyValue(props.get(property)));
            }
        }
    ...
    private Serializable toAlfredApiPropertyValue(Serializable value) {
        if (value instanceof NodeRef) {
            NodeRef nodeRef = (NodeRef) value;
            return c.alfredApi(nodeRef);
        }
        if (value instanceof List) {
            ArrayList<Serializable> list = new ArrayList<>((List) value);
            ArrayList<Serializable> result = new ArrayList<>();
            for (Serializable item : list) {
                result.add(toAlfredApiPropertyValue(item));
            }
            return result;
        }
        return value;
