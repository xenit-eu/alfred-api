...
public class Reader {
    ...
    private String[] ignoredRoutes = new String[0];
...
//    }
    ...
    private static boolean isValidResponse(Type type) {
        ...
        //TODO: return !javax.ws.rs.core.Response.class.isAssignableFrom(cls) && !isResourceClass(cls);
    ...
    public Swagger read(Set<Class<?>> classes) {
        ...
        for (Class<?> cls : classes) {
            read(cls, "", null, false, new String[0], new String[0], new HashMap<String, Tag>(),
    ...
    protected Swagger read(Class<?> cls, String parentPath, String parentMethod, boolean isSubresource,
        ...
        return read(cls, parentPath, parentMethod, isSubresource, parentConsumes, parentProduces, parentTags,
                parentParameters, new HashSet<Class<?>>());
    }
    ...
    private Swagger read(Class<?> cls, String parentPath, String parentMethod, boolean isSubresource,
        ...
        boolean hasPathAnnotation = false;//TODO:  (ReflectionUtils.getAnnotation(cls, javax.ws.rs.Path.class) != null);
        boolean hasApiAnnotation = (api != null);
        boolean isApiHidden = hasApiAnnotation && api.hidden();
        boolean hasDEWebscript = webscript != null;
        ...
        boolean readable = classReadable || hasDEWebscript;//&& config.isScanAllResources());
        ...
        Tag webscriptTag = null;

        if (webscript != null) {

            parentPath += webscript.baseUri();

            Tag tag = new Tag();
            tag.setDescription(webscript.description());

//            cls.getAnnotation(webscript)
//            String webscriptTagName = cls.getSimpleName();
//            String[] cutOff = {"webscript","webscript1"};
//            for(int i =0;i<cutOff.length;i++) {
//                String currentCut = cutOff[i];
//                if (webscriptTagName.toLowerCase().endsWith(currentCut))
//                    webscriptTagName = webscriptTagName.substring(0, webscriptTagName.length() - currentCut.length());
//                tag.setName(webscriptTagName);
//            }
//
            boolean tagHasValidNameProperty = webscript.value() != null && webscript.value().length() > 0;
            String name = tagHasValidNameProperty ? webscript.value() : cls.getSimpleName();
            tag.setName(name);
            swagger.addTag(tag);
            webscriptTag = tag;
        ...
        if (hasApiAnnotation && api.tags() != null && api.tags().length > 0) {
            webscriptTag = null; // Don't use webscripttags when custom is specified
        }
        ...
        if (readable) {
            ...
            for (Method method : methods) {
                ...
                String operationPath = getPath(method, parentPath);
                ...
                if (operationPath != null) {
                    ...
                    if (httpMethod != null) {
                        ...
                        if (webscriptTag != null) {
                            operation.tag(webscriptTag.getName());
                        }
    ...
    protected Parameter readImplicitParam(ApiImplicitParam param) {
        ...
        try {
            if (param.dataType().equals("file")) {
                type = java.io.File.class;
            } else {
                type = Class.forName(param.dataType());
            }
        } catch (ClassNotFoundException e) {
            LOGGER.debug("Class not found, using native swagger method. dataType: " + param.dataType());
...
//    }
    ...
    private String getPath(Method method, String parentPath) {
        // DE
        ...
        if (uri.value().length != 1) {
            throw new RuntimeException("Should have a single path for uri!" + uri.value());
        }
        return parentPath + uri.value()[0];
//
//        if (classLevelPath == null && methodLevelPath == null && StringUtils.isEmpty(parentPath)) {
//            return null;
//        }
                    ...
                    if (!isVoid(cls)) {
                        ...
                        if (property != null) {
                            Property responseProperty = ContainerWrapper
                                    .wrapContainer(header.responseContainer(), property,
...
//        }
        ...
        LocalVariableTableParameterNameDiscoverer disco = new LocalVariableTableParameterNameDiscoverer();
        String[] parameterNames = disco.getParameterNames(method);
...
//            return httpMethod.value().toLowerCase();
        ...
        Uri uri = (Uri) method.getAnnotation(Uri.class);
        if (uri != null) {
            return uri.method().toString().toLowerCase();
        }
    ...
    private boolean isIgnored(String path) {
        for (String item : getIgnoredRoutes()) {
