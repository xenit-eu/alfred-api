...
public class ApixToAlfrescoConversion {
    ...
    public NodeRef alfresco(eu.xenit.apix.data.NodeRef obj) {
        return new NodeRef(obj.getValue());
    }

    public StoreRef alfresco(eu.xenit.apix.data.StoreRef obj) {
        return new StoreRef(obj.getValue());
    }

    public eu.xenit.apix.data.NodeRef apix(NodeRef obj) {
        if (obj == null) {
            return null;
        }
        return new eu.xenit.apix.data.NodeRef(obj.toString());
    }

    public List<eu.xenit.apix.data.NodeRef> apix(List<NodeRef> obj) {
        //return obj.stream().map(n->apix(n)).collect(Collectors.toList());
        List<eu.xenit.apix.data.NodeRef> apixNodes = new ArrayList<>(obj.size());
...
//    }
    ...
    public Set<eu.xenit.apix.data.NodeRef> apixNodeRefs(Set<NodeRef> obj) {
        Set<eu.xenit.apix.data.NodeRef> apixNodes = new HashSet<>(obj.size());
        for (NodeRef alfNode : obj) {
            apixNodes.add(apix(alfNode));
        }
    ...
    public List<NodeRef> alfresco(List<eu.xenit.apix.data.NodeRef> obj) {
        //return obj.stream().map(n->alfresco(n)).collect(Collectors.toList());

        List<NodeRef> alfNodes = new ArrayList<>(obj.size());
        for (eu.xenit.apix.data.NodeRef apixNode : obj) {
    ...
    public Set alfresco(Set s) {
        ...
        if (first instanceof eu.xenit.apix.data.NodeRef) {
            return alfrescoNoderefs(s);
        } else if (first instanceof eu.xenit.apix.data.QName) {
    ...
    public Set<NodeRef> alfrescoNoderefs(Set<eu.xenit.apix.data.NodeRef> obj) {
    ...
    public boolean HasAlfrescoQname(eu.xenit.apix.data.QName qName) {
        try {
    ...
    public boolean HasApixQname(org.alfresco.service.namespace.QName qName) {
        try {
            eu.xenit.apix.data.QName apixName = apix(qName);
    ...
    public QName alfresco(eu.xenit.apix.data.QName obj) {
    ...
    public ContentData alfresco(eu.xenit.apix.data.ContentData obj) {
        return new ContentData(obj.getContentUrl(), obj.getMimetype(), obj.getSize(), obj.getEncoding(),
                obj.getLocale());
    }


    public Path alfresco(eu.xenit.apix.data.Path obj) {
    ...
    public Set<QName> alfrescoQNames(Set<eu.xenit.apix.data.QName> obj) {
        Set<QName> alfQnames = new HashSet<>(obj.size());
        for (eu.xenit.apix.data.QName apixQname : obj) {
    ...
    public Set<eu.xenit.apix.data.QName> apixQNames(Set<QName> obj) {
        Set<eu.xenit.apix.data.QName> apixQnames = new HashSet<>(obj.size());
        for (QName qname : obj) {
    ...
    public eu.xenit.apix.data.QName apix(QName obj) {
        return new eu.xenit.apix.data.QName(obj.toString());
    }
}
